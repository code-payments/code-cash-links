...
import { Kin, Hash, Solana } from '../../utils';
import { NonceAccount } from "./NonceAccount";
import { getCommitmentPda, getCommitmentVaultPda } from "./Commitment";
import { protoToAccountType, TimelockAccount } from "./TimelockAccount";
...
export interface Action {
    ...
    toProto(): Promise<proto.Action>;
    updateFromServerResponse(params: proto.ServerParameter): Promise<void>;
...
export class BaseAction implements Action {
    ...
    async setNonceFromServerResponse(params: proto.ServerParameter): Promise<void> {
    ...
    async toProto(): Promise<proto.Action> {
        throw new Error("Method not implemented on abstract class.");
    }

    async updateFromServerResponse(params: proto.ServerParameter): Promise<void> {
        throw new Error("Method not implemented on abstract class.");
    }
...
export class OpenAccountAction extends BaseAction implements Action {
    ...
    async toProto(): Promise<proto.Action> {
        const openAccount = new proto.OpenAccountAction({
            ...
            index: BigInt(this.account.getDerivationIndex()),
        })

        const buf = openAccount.toBinary();
        ...
        // Set the signature
        openAccount.authoritySignature = new proto.Common.Signature({ value: Buffer.from(sig) });

        // Return the action
        return new proto.Action({
            type: { case: "openAccount", value: openAccount, },
            id: this.getId()
        })
    }
}
...
export class CloseDormantAccountAction extends BaseAction implements Action {
    ...
    async getSignatures(env: Environment): Promise<Uint8Array[]> {
        ...
        const sig = await Solana.signTransaction(tx, this.account.authority);
        return [sig];
    }

    async toProto(): Promise<proto.Action> {
        const closeDormantAccount = new proto.CloseDormantAccountAction({
            accountType: this.account.accountType as number,
            authority: { value: this.account.getAuthority().toBuffer(), },
            token: { value: this.account.getVault().toBuffer(), },
        ...
        return new proto.Action({
            type: { case: "closeDormantAccount", value: closeDormantAccount, },
            id: this.getId()
        });
    }
}
...
export class CloseEmptyAccountAction extends BaseAction implements Action {
    ...
    async toProto(): Promise<proto.Action> {
        const closeEmptyAccount = new proto.CloseEmptyAccountAction({
            accountType: this.account.accountType as number,
            authority: { value: this.account.getAuthority().toBuffer(), },
            token: { value: this.account.getVault().toBuffer(), },
        });
        ...
        return new proto.Action({
            type: { case: "closeEmptyAccount", value: closeEmptyAccount, },
            id: this.getId()
        });
    }
}
...
export class NoPrivacyWithdrawAction extends CloseDormantAccountAction implements Action {
    ...
    async toProto(): Promise<proto.Action> {
        const noPrivacyWithdraw = new proto.NoPrivacyWithdrawAction({
            authority: { value: this.account.getAuthority().toBuffer(), },
        ...
        return new proto.Action({
            type: { case: "noPrivacyWithdraw", value: noPrivacyWithdraw, },
            id: this.getId()
        });
    }
}
...
export class TemporaryPrivacyTransferAction extends BaseAction implements Action {
    ...
    async updateFromServerResponse(data: proto.ServerParameter): Promise<void> {
        await this.setNonceFromServerResponse(data);

        if (data.type.case != "temporaryPrivacyTransfer") {
            throw new Error("Invalid server response");
        }

        const params = data.type.value as proto.TemporaryPrivacyTransferServerParameter;
        if (params == undefined) {
            console.log(data);
            throw new Error("Invalid server response");
        }
        ...
        await this.handleServerResponse(Buffer.from(treasury.value), Buffer.from(recentRoot.value));
    }
    ...
    getTranscript(): string {
        ...
        const transcript = [
            ...
            `transfer ${Kin.ToQuarks(this.amount)} quarks `,
    ...
    getTranscriptHash(): Buffer {
        return Buffer.from(Hash.sha256(Buffer.from(this.getTranscript())));
    }
    ...
    async toProto(): Promise<proto.Action> {
        const temporaryPrivacyTransfer = new proto.TemporaryPrivacyTransferAction({
            authority: { value: this.account.getAuthority().toBuffer(), },
            source: { value: this.account.getVault().toBuffer(), },
            destination: { value: this.destination.toBuffer(), },
            amount: BigInt(Kin.ToQuarks(this.amount)),
        });

        return new proto.Action({
            type: { case: "temporaryPrivacyTransfer", value: temporaryPrivacyTransfer, },
            id: this.getId()
        });
    }
}
...
export class TemporaryPrivacyExchangeAction extends TemporaryPrivacyTransferAction implements Action {
    ...
    async toProto(): Promise<proto.Action> {
        const temporaryPrivacyExchange = new proto.TemporaryPrivacyExchangeAction({
            authority: { value: this.account.getAuthority().toBuffer(), },
            source: { value: this.account.getVault().toBuffer(), },
            destination: { value: this.destination.toBuffer(), },
            amount: BigInt(Kin.ToQuarks(this.amount)),
        ...
        return new proto.Action({
            type: { case: "temporaryPrivacyExchange", value: temporaryPrivacyExchange, },
            id: this.getId()
        });
    }
    ...
    async updateFromServerResponse(data: proto.ServerParameter): Promise<void> {
        ...
        if (data.type.case != "temporaryPrivacyExchange") {
            throw new Error("Invalid server response");
        }

        const params = data.type.value as proto.TemporaryPrivacyExchangeServerParameter;
        if (params == undefined) {
            console.log(data);
            throw new Error("Invalid server response");
        }
...
export class PermanentPrivacyUpgradeAction extends BaseAction implements Action {
    ...
    static async fromUpgradeIntent(
        env: Environment,
        intent: proto.UpgradeableIntent,
        actionIndex: number
        ...
        action.intentId = Buffer.from(intent.id?.value!);

        const treasury = intent.actions[actionIndex].treasury;
        const recentRoot = intent.actions[actionIndex].recentRoot;
        const serverParam = new proto.ServerParameter({
            ...
            type: {
                ...
                value: new proto.TemporaryPrivacyTransferServerParameter({
                    treasury, recentRoot,
                })
            },
        });
        ...
        await action.updateFromServerResponse(serverParam);
        ...
        const localMsg = Solana.compileMessage(localTx);
    ...
    static async parseUpgradeIntentProto(env: Environment, intent: proto.UpgradeableIntent, actionIndex: number) {
        ...
        const source = await TimelockAccount.deriveFrom(env, Number(sourceDerivationIndex), sourceAccountType);
        ...
        const amount = Kin.FromQuarks(Number(data.originalAmount))
    ...
    isValid(): boolean {
        ...
        let computed_hash = Hash.sha256(leaf);
        for (let proof_element of proof) {
            if (Buffer.compare(computed_hash, proof_element) <= 0) {
                // Hash(current computed hash + current element of the proof)
                computed_hash = Hash.sha256(Buffer.concat([computed_hash, proof_element]));
            } else {
                // Hash(current element of the proof + current computed hash)
                computed_hash = Hash.sha256(Buffer.concat([proof_element, computed_hash]));
            }
    ...
    async updateFromServerResponse(data: proto.ServerParameter): Promise<void> {
        if (data.type.case != "permanentPrivacyUpgrade") {
            throw new Error("Invalid server response");
        }

        const params = data.type.value as proto.PermanentPrivacyUpgradeServerParameter;
        if (params == undefined) {
            throw new Error("Invalid server response");
        }
        ...
        this.merkleRoot = Buffer.from(merkleRoot.value);
        this.merkleProof = merkleProof.map((x) => Buffer.from(x.value));
        this.newCommitment = new PublicKey(newCommitment.value);
        this.newCommitmentAmount = Number(params.newCommitmentAmount);
        this.newCommitmentDestination = new PublicKey(newCommitmentDestination.value);
        this.newCommitmentTranscript = Buffer.from(newCommitmentTranscript.value);
    ...
    async getSignatures(env: Environment): Promise<Uint8Array[]> {
        const tx = await this.getTransaction(env);
        const sig = await Solana.signTransaction(tx, this.temporaryTransfer.account.authority);
        return [sig];
    }

    async toProto(): Promise<proto.Action> {
        const permanentPrivacyUpgrade = new proto.PermanentPrivacyUpgradeAction({
            actionId: this.temporaryTransfer.getId(),
        });

        return new proto.Action({
            type: { case: "permanentPrivacyUpgrade", value: permanentPrivacyUpgrade, },
            id: this.getId()
        });
    }
}
...
export async function getUpgradeReqProto(owner: Keypair, limit: number = DefaultUpgradeReqLimit)
    : Promise<proto.GetPrioritizedIntentsForPrivacyUpgradeRequest> {

    const req = new proto.GetPrioritizedIntentsForPrivacyUpgradeRequest({
